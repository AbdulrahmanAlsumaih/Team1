...
def handle_found_object(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if (
            (len(png_files) != num_renders+1)
            or (len(npy_files) != num_renders+1)
            or (len(metadata_files) != 1)
        ):
            logger.error(
                f"Found object {file_identifier} was not rendered successfully!" + ' len png: ' + str(len(png_files)) + ' num renders+1: ' + str(num_renders+1) +' len npy: ' + str(len(npy_files))
            )
...
def get_example_objects() -> pd.DataFrame:
  ...
  # while True:
  #   samples = annotations.sample(1)
  #   if samples.iloc[0]['source'] == 'github':
  #     break
  samples = annotations.sample(10)
  samples = samples.loc[samples['source'] != 'thingiverse']
  sample_list = []
  for i in range(len(samples)): # iterate over index of the DataFrame
    row = samples.iloc[i] # get the row at index i
    if row['source'] == 'github':
      ...
      temp_dict['fileIdentifier'] = fileIdentifier
      temp_dict['source'] = source
      sample_list.append(temp_dict)

  with open('samples.json', 'w') as fout:
    json.dump(sample_list , fout)

  return pd.read_json("samples.json", orient="records")
