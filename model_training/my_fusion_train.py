...
from my_gaussian_predictor import Fusion_GaussianSplatPredictor
...
def main(cfg: DictConfig):
    ...
    dataset = z123Dataset("/home/cap6411.student1/CVsystem/final_project/splatter-image/my_train_dataset/train_split", cfg)
    ...
    val_dataset = z123Dataset("/home/cap6411.student1/CVsystem/final_project/splatter-image/my_train_dataset/val_split", cfg)
    ...
    test_dataset = z123Dataset("/home/cap6411.student1/CVsystem/final_project/splatter-image/my_train_dataset/test_split", cfg)
    test_dataloader = DataLoader(test_dataset, 
                                 batch_size=1,
                                 shuffle=True)
   
    fusion_gaussian_predictor = Fusion_GaussianSplatPredictor(gaussian_predictor, cfg)
    fusion_gaussian_predictor = fusion_gaussian_predictor.to("cuda")
    #fusion_gaussian_predictor = torch.nn.parallel.DistributedDataParallel(fusion_gaussian_predictor, find_unused_parameters=True)
    # distribute model and training dataset
    fusion_gaussian_predictor, optimizer = fabric.setup(
        fusion_gaussian_predictor, optimizer
    )
    dataloader = fabric.setup_dataloaders(dataloader)
    
    fusion_gaussian_predictor.train()
    ...
    for num_epoch in range((cfg.opt.iterations + 1 - first_iter)// len(dataloader) + 1):
        ...
        for data in pbar:
            ...
            #print("in main input_images shape:", input_images.shape)
            gaussian_splats = fusion_gaussian_predictor(input_images,
            ...
            fusion_gaussian_predictor.eval()
            ...
            with torch.no_grad():
                ...
                if (iteration % cfg.logging.loop_log == 0 or iteration == 1) and fabric.is_global_zero:
                    ...
                    gaussian_splats_vis = fusion_gaussian_predictor(input_images,
            ...
            for fname_to_save in fnames_to_save:
                ...
                if cfg.opt.ema.use:
                    ...
                    ckpt_save_dict["model_state_dict"] = fusion_gaussian_predictor.state_dict() 
