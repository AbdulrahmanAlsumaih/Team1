...
@torch.no_grad()
def evaluate_dataset(model, dataloader, device, model_cfg, save_vis=0, out_folder=None, fusion=False
                     ):
    """
    ...
    for d_idx, data in enumerate(tqdm.tqdm(dataloader)):
        ...
        if model_cfg.data.origin_distances:
            if fusion==False:
                ...
                input_images = torch.cat([data["gt_images"],
                                      data["origin_distances"]])
        else:
            if fusion==False:
